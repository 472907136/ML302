...
#include "../../../../../../../oc/kernel/os/include/sys/errno.h"
...
int cmmqtt_read(CMMQTT_Network *n, unsigned char *buffer, int len, uint32_t timeout_ms)
{
	...
	cmmqtt_log("mqtt read select=%d", ret);
	//add cmiot ssl,don't select while ssl reading, or it fails
	if (n->wlfssl != NULL)
	{ret=1;}
	if (ret < 0)
	{
	...
	else if (ret == 1)
    {
        do
		{
			if (n->wlfssl != NULL)
			{
				cmmqtt_log("wolfSSL_read,len=%d",len);
				//n->wlfssl->rflags = MSG_DONTWAIT;
				rc = wolfSSL_read(n->wlfssl, buffer + recvlen, len - recvlen);
			}
			else
...
int cmmqtt_write(CMMQTT_Network *n, unsigned char *buffer, int len, uint32_t timeout_ms)
{
	...
	else if (ret == 1)
    {
    	if (n->wlfssl != NULL)
    	{
    		cmmqtt_log("wolfSSL_write");
			rc = wolfSSL_write(n->wlfssl, buffer, len);
    	}
		else
