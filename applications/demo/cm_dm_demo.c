...
{
    cm_dm_event_t *pEvent = (cm_dm_event_t *)param;
    st_context_t *context = (st_context_t *)pEvent->param1;
    struct cm_dm_st_callback_info *newNode = (struct cm_dm_st_callback_info *)pEvent->param2;
...
static void dm_PostEvent(void *context, cm_dm_callback_type_t event_id, void *param)
{
    cm_dm_event_t *ev = (cm_dm_event_t *)malloc(sizeof(cm_dm_event_t));
    ev->id = event_id;
    ev->param1 = (uint32_t)context;
    ev->param2 = (uint32_t)param;
    osiThreadCallback(g_dm_hThread, (cm_dm_callback_t)dm_event_ind_process, ev);
}
...
{
    struct cm_dm_st_callback_info *newNode = (struct cm_dm_st_callback_info *)malloc(sizeof(struct cm_dm_st_callback_info));
    newNode->next = NULL;
...
void cm_test_dm_register(unsigned char **cmd,int len)
{
	...
	cm_printf("[DM]dm_register isAdapterPlatform = %d !!\r\n", isAdapterPlatform);
    ...
    if (isAdapterPlatform)
    {
        ...
        {
            cm_printf("[DM]cis entry init failed.\r\n");
            return;
        }
		
		cm_printf("[DM]dm_register dm_config.szCMEI_IMEI = %s!!\r\n", dm_config.szCMEI_IMEI);
	    //cm_printf("dm_register dm_config.szIMSI = %s!!\r\n", dm_config.szIMSI);
	    //cm_printf("dm_register dm_config.szDMv = %s\r\n!!", dm_config.szDMv);
	    //cm_printf("dm_register dm_config.szAppKey = %s\r\n!!", dm_config.szAppKey);	
	    //cm_printf("dm_register dm_config.szPwd = %s!!\r\n", dm_config.szPwd);
    }
    else
    {
   ...
   // g_dm_pumpSema = osSemaphoreCreate(1, 0);
    ...
    {
        cm_printf("[DM]register success.\n");
    }
    else
    {
        cm_printf("[DM]register fail.\n");
