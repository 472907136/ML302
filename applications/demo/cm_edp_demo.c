...
int32 DoSend(int32 sockfd, const char *buffer, uint32 len)
{
    ...
    while (len != total)
    {
        ...
        if (n <= 0)
        {
            cm_printf("[EDP][ERROR]: writing to socket\n");
            return n;
        }
        /* 成功发送了n个字节的数据 */
        total += n;
        cm_printf("[EDP]:send %d bytes\n", n);
    }
...
void recv_thread_func(void *arg)
{
    ...
    cm_printf("[EDP]: recv thread start ...\n");
    ...
    while (error == 0)
    {
        ...
        if(ret < 0)
        {
            cm_printf("[EDP]: %s() read select error ,ret=%d\n", __func__, ret);
        ...
        else
        {
            ...
            {
                cm_printf("[EDP]: %s() tcp_exptfds error\n", __func__);
                Close(g_sockfd);
                break;
            }
            ...
            if(0 == FD_ISSET(g_sockfd, &iot_tcp_readfds))
            {
                ...
                cm_printf("[EDP]: %s() tcp_readfds unkwon error\n", __func__);
                osDelay(1000);
                continue;
            }
            ...
            if (n <= 0)
            {
                cm_printf("[EDP]: recv from server, error");
                break;
            }
            cm_printf("[EDP]:recv from server, bytes: %d\n", n);
            /* wululu test print send bytes */
            ...
            while (1)
            {
                ...
                case CONNRESP:
                    ...
                    cm_printf("[EDP]:recv connect resp, rtn: %d\n", rtn);
                    break;
                case PUSHDATA:
                    /* 解析EDP包 - 数据转发 */
                    UnpackPushdata(pkg, &src_devid, &push_data, &push_datalen);
                    cm_printf("[EDP]:recv push data, src_devid: %s, push_data: %s, len: %d\n",
                ...
                case UPDATERESP:
                    ...
                    while (up_info)
                    {
                        cm_printf("[EDP]:name = %s\n", up_info->name);
                        cm_printf("[EDP]:version = %s\n", up_info->version);
                        cm_printf("[EDP]:url = %s\nmd5 = ", up_info->url);
                        for (i = 0; i < 32; ++i)
                        {
                            cm_printf("%c", (char)up_info->md5[i]);
                        }
                        cm_printf("\n");
                ...
                case SAVEDATA:
                    ...
                    if (UnpackSavedata(pkg, &src_devid, &jsonorbin) == 0)
                    {
                        ...
                        { /* 解析EDP包 - bin数据存储 */
                            ...
                            cm_printf("[EDP]:recv save data bin, src_devid: %s, desc json: %s, bin: %s, binlen: %d\n",
                        ...
                        else if (jsonorbin == kTypeStringWithTime)
                        {
                            ...
                            cm_printf("[EDP]:time:%u-%02d-%02d %02d-%02d-%02d\nstr val:%s\n",
                        ...
                        else if (jsonorbin == kTypeFloatWithTime)
                        {
                            ...
                            {
                                cm_printf("[EDP]:UnpackSavedataFloatWithTime failed!\n");
                            }

                            cm_printf("[EDP]:read time:%u-%02d-%02d %02d-%02d-%02d\n",
                                   stTime.year, stTime.month, stTime.day, stTime.hour, stTime.minute, stTime.second);
                            cm_printf("[EDP]:read float data count:%d, ptr:[%p]\n", count, float_data);

                            for (i = 0; i < count; ++i)
                            {
                                cm_printf("[EDP]:ds_id=%u,value=%f\n", float_data[i].ds_id, float_data[i].f_data);
                            }
                    ...
                    {
                        cm_printf("[EDP]: SAVEDATA error\n");
                    }
                    break;
                case SAVEACK:
                    UnpackSavedataAck(pkg, &msg_id, &save_date_ret);
                    cm_printf("[EDP]:save ack, msg_id = %d, ret = %d\n", msg_id, save_date_ret);
                    break;
                case CMDREQ:
                                     ...
                                     &cmd_req, &cmd_req_len) == 0)
                    {
                         ...
                         */
                         ...
                         cm_printf("[EDP]:recvlen(%d),recvdata(%s)\n", cmd_req_len,RecvCmdStr);
                ...
                case PINGRESP:
                    /* 解析EDP包 - 心跳响应 */
                    UnpackPingResp(pkg);
                    cm_printf("[EDP]:recv ping resp\n");
                    break;

                default:
                    /* 未知消息类型 */
                    error = 1;
                    cm_printf("[EDP]:recv failed...\n");
                    break;
                }
                DeleteBuffer(&pkg);
            }
...
void cm_test_edp(unsigned char **cmd,int len)
{
    ...
    {
        //exit(0);
        cm_printf("[EDP][ERROR]:open socket fail\n");
    }
    //cm_printf("Open sock successed sockfd=%d\n", g_sockfd);
    ...
    if (g_is_encrypt)
    {
        ...
        /* 向设备云发送加密请求 */
        //cm_printf("send encrypt to server, bytes: %d\n", send_pkg->_write_pos);
    ...
    /* 向设备云发送连接请求 */
    //cm_printf("send connect to server, bytes: %d\n", send_pkg->_write_pos);
    ret = DoSend(g_sockfd, (const char *)send_pkg->_data, send_pkg->_write_pos);
    DeleteBuffer(&send_pkg);
    ...
    //cm_printf("send simple format (string)\n");
    DoSend(g_sockfd, (const char *)send_pkg->_data, send_pkg->_write_pos);
    DeleteBuffer(&send_pkg);
    cm_printf("[EDP]:send string\n");
