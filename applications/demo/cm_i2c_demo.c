...
void BMP_ReadCalibrationData()
{
	...
	memset(temp,0,3);
	unsigned char reg;
	cm_i2c_slave_t slave;

	reg = 0xAA;
	slave.addr_device = 0x77;
	slave.addr_data = reg;
	slave.addr_data_low = 0;
	slave.reg_16bit = false;
	cm_i2c_write(CM_I2C_MASTER_2, &slave, &reg, 1);
	cm_i2c_read(CM_I2C_MASTER_2, &slave, temp, 2);
...
long BMP_Read_UT(void)
{
	...
	buffer[1] = 0x2E;
	slave.addr_device = 0x77;
	slave.addr_data = buffer[1];
	slave.addr_data_low = buffer[0];
	slave.reg_16bit = true;
	cm_i2c_write(CM_I2C_MASTER_2, &slave,buffer,2);	
	vTaskDelay(1);
	memset(buffer,0,3);
	reg = 0xF6;
	slave.addr_data = reg;
	...
	slave.reg_16bit = false;
	cm_i2c_write(CM_I2C_MASTER_2, &slave, &reg, 1);
	cm_i2c_read(CM_I2C_MASTER_2, &slave, buffer, 2);
...
int BMP180_get_id()
{
	...
	if(0 != cm_i2c_init(CM_I2C_MASTER_2, CM_I2C_BPS_100K))
	{
		return -1;
	}
	memset(id,0,2);
	...
	slave.reg_16bit = false;
	cm_i2c_write(CM_I2C_MASTER_2, &slave, &reg, 1);
	cm_i2c_read(CM_I2C_MASTER_2, &slave, id, 1);
	if(id[0] != 0)
	{
		cm_printf("[I2C]ID:%d\n",id[0]);
		return 0;
...
void cm_test_i2c_bmp180(unsigned char **cmd,int len)
{
	...
	if(BMP180_get_id())
	{
		cm_printf("[I2C]no bmp180 found!\n");
	}
	BMP_ReadCalibrationData();
	for(i=0;i<10;i++)
	{
		BMP_UncompemstatedToTrue();
		cm_printf("[I2C]temp = %d.%dC\t   Pressure = %ldPa\t   Altitude = %.5fm\r\n",bmp180.Temp/10,bmp180.Temp%10,bmp180.p,bmp180.altitude);
		vTaskDelay(100);
	}
	cm_i2c_deinit(CM_I2C_MASTER_2);
}
