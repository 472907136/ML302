...
/* ---------------------------------------------------------------------------*/
  ...
  defined(FREESCALE_FREE_RTOS)
    ...
    int wc_UnLockMutex(wolfSSL_Mutex* m)
    {
		sxr_ReleaseMutex(m->id, m->user_id);
		return 0;
    }
...
{
#if 1
	/*TM_FILETIME fileTime = {0};
	time_t tmp = 0;

	TM_GetSystemFileTime(&fileTime);
	tmp = fileTime.DateTime + 946656000;  //RDA FILE TIME is the seconds from 2000 to now
	...
	return tmp;*/
	time_t tmp = 0;
	tmp=osiEpochSecond()+cm_get_timezone()* 15*60;
	return tmp;
#endif
	//return (time_t)NULL;
}
