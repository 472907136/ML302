 ...
 */
typedef void (*cm_vir_at_cb)(void *param);
typedef enum cm_boot_cause
{
    CM_BOOTCAUSE_UNKNOWN = 0,           ///0,< placeholder for unknown reason
    CM_BOOTCAUSE_PWRKEY = (1 << 0),     ///1,< boot by power key
    CM_BOOTCAUSE_PIN_RESET = (1 << 1),  ///2,< boot by pin reset
    CM_BOOTCAUSE_ALARM = (1 << 2),      ///4,< boot by alarm
    CM_BOOTCAUSE_CHARGE = (1 << 3),     ///8,< boot by charge in
    CM_BOOTCAUSE_WDG = (1 << 4),        ///16,< boot by watchdog
    CM_BOOTCAUSE_PIN_WAKEUP = (1 << 5), ///32,< boot by wakeup
    CM_BOOTCAUSE_PSM_WAKEUP = (1 << 6), ///64,< boot from PSM wakeup
} cm_boot_cause_t;
typedef struct cm_cmd {
    unsigned char * cmdstr;
    void (* cmdfunc)(char **,int);
} cm_cmd_t;
#define cm_sys_log(fmt,arg...)  sys_arch_printf("[CM_OC]"fmt,##arg);
/**
 *  \brief us级精确延时接口
 *  
 ...
 */
void cm_sys_delay_us(int delay_us);
/**
 *  \brief 获取模组启动原因
 *  
 *  \return 模组启动原因
 ...
 *  \details More details
 */
cm_boot_cause_t cm_sys_get_boot_cause(void);
/**
 *  \brief 重启模组
 *  
 *  \return 空
 ...
 */
int cm_vir_at_init(cm_vir_at_cb cb);
/**
 *  \brief 虚拟AT通道发送接口
 *  
 ...
 */
int cm_vir_at_send(unsigned char * at_str);
/**
 *  \brief 虚拟AT通道接收函数
 *  
 ...
 *  \details AT虚拟通道仅支持ML302 AT文档中第2到8章包含的AT命令
 */
int cm_vir_at_get(unsigned char *buf,int max_len);
/**
 *  \brief 关闭AT虚拟通道
 *  
 *  \return 空
